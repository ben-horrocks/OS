...
int P1_shellTask(int argc, char* argv[])
{
	...
	sigAction(defaultSigIntHandler, mySIGINT);
	...
	while (1)
	{
		...
		// ?? must use malloc for argv storage!
		{
            ...
            newArgv = calloc(newArgc + 1, sizeof(char*));
			for (i=0;i<newArgc;i++)
            {
//                printf("length: %d\n", strlen(myArgv[i]));
			    char *newarg = calloc(strlen(myArgv[i])+1, sizeof(char));
                strcpy(newarg, myArgv[i]);
			    if (newarg[0] != '"' || newarg[strlen(newarg)-1] != '"')
                {
			        int x = 0;
                    while(newarg[x] != '\0')
                    {
                        newarg[x++] = tolower(newarg[x]);
                    }
                }
                if(i == newArgc - 1 && newarg[strlen(newarg) - 1] == '&')
                {
                    background = TRUE;
		...
		for (found = i = 0; i < NUM_COMMANDS; i++)
		{
				 ...
				 !strcmp(newArgv[0], commands[i]->shortcut))
			{
				...
				if(background == TRUE)
                {
//					printf("Execute Background Task");
...
Command** P1_init()
{
    ...
    commands[i++] = newCommand("add", "add", add, "add all numbers in command line (decimal or hexadecimal)");
    commands[i++] = newCommand("args", "echo", args, "list all parameters on the command line, numbers or strings");
