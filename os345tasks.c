...
//        printf("CreateTask loop\n");
		if (tcb[tid].name == 0)
		{
			...
			//			printf("Starting allocation of stuff\n");
			tcb[tid].argv = calloc(argc + 1, sizeof(char*));
			for (int i=0;i<argc;i++)
			{
				char *newarg = calloc(strlen(argv[i])+1, sizeof(char));
